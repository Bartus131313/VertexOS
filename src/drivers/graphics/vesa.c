...
void vesa_init(multiboot_info_t* mbi) {
    ...
    vesa_screen_width = mbi->framebuffer_width;
    vesa_screen_height = mbi->framebuffer_height;
    vesa_screen_pitch = mbi->framebuffer_pitch;

    // Allocate memory for our back buffer so we don't flicker
    // width * height * 4 bytes per pixel
    back_buffer = (uint32_t*)kmalloc(vesa_screen_width * vesa_screen_height * 4);
}

uint32_t vesa_get_screen_width() { return vesa_screen_width; }
uint32_t vesa_get_screen_height() { return vesa_screen_height; }
void vesa_get_screen_size(uint32_t* width, uint32_t* height) {
    *width = vesa_screen_width;
    *height = vesa_screen_height;
}
...
void vesa_draw_pixel(int x, int y, uint32_t color) {
    back_buffer[y * vesa_screen_width + x] = color;
}
...
void vesa_draw_mouse(int x, int y) {
    for (int i = 0; i < 19; i++) {
        for (int j = 0; j < 12; j++) {
            if (mouse_cursor[i][j] == 1) vesa_draw_pixel(x + j, y + i, RGB(255, 255, 255));
            else if (mouse_cursor[i][j] == 2) vesa_draw_pixel(x + j, y + i, RGB(0, 0, 0));
        }
    }
}